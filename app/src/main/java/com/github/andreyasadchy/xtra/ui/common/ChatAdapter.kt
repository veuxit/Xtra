...
class ChatAdapter(
    ...
    private fun loadImages(holder: ViewHolder, images: List<Image>, builder: SpannableStringBuilder, message: String?, userId: String?, userName: String?, channelId: String?, fullMsg: String?) {
        images.forEach {
            when (imageLibrary) {
                "0" -> loadCoil(holder, it, builder, message, userId, userName, channelId, fullMsg)
                "1" -> {
                    when {
                        it.format.equals("webp", true) -> loadGlide(holder, it, builder, message, userId, userName, channelId, fullMsg)
                        else -> loadCoil(holder, it, builder, message, userId, userName, channelId, fullMsg)
                    }
                }
                else -> loadGlide(holder, it, builder, message, userId, userName, channelId, fullMsg)
            }
        }
    }

    private fun loadCoil(holder: ViewHolder, image: Image, builder: SpannableStringBuilder, message: String?, userId: String?, userName: String?, channelId: String?, fullMsg: String?) {
