...
class VideoSearchViewModel @Inject constructor(
    ...
    override val result: LiveData<Listing<Video>> = Transformations.map(query) {
        repository.loadSearchVideos(it, gqlClientId.value?.nullIfEmpty(), viewModelScope)
    }

    fun setQuery(query: String, gqlClientId: String? = null) {
    ...
    fun saveBookmark(context: Context, video: Video) {
        GlobalScope.launch {
            ...
            if (item != null) {
                ...
                val userTypes = video.channelId?.let { repository.loadUsersById(mutableListOf(it), null, null, gqlClientId.value) }?.first()
