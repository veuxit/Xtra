...
class ChannelVideosDataDeserializer : JsonDeserializer<ChannelVideosDataResponse> {
    ...
    override fun deserialize(json: JsonElement, typeOfT: Type, context: JsonDeserializationContext): ChannelVideosDataResponse {
        ...
        val dataJson = json.asJsonObject.get("data").asJsonObject.get("user").asJsonObject.get("videos").asJsonObject.get("edges").asJsonArray
        ...
        val hasNextPage = json.takeIf { it.isJsonObject }?.asJsonObject?.get("data")?.takeIf { it.isJsonObject }?.asJsonObject?.get("user")?.takeIf { it.isJsonObject }?.asJsonObject?.get("videos")?.takeIf { it.isJsonObject }?.asJsonObject?.get("pageInfo")?.takeIf { it.isJsonObject }?.asJsonObject?.get("hasNextPage")?.takeIf { it.isJsonPrimitive }?.asJsonPrimitive?.takeIf { it.isBoolean }?.asBoolean
        dataJson?.forEach { item ->
            item.takeIf { it.isJsonObject }?.asJsonObject?.get("node")?.takeIf { it.isJsonObject }?.asJsonObject?.let { obj ->
                ...
                data.add(Video(
                    ...
                    uploadDate = obj.get("publishedAt")?.takeIf { it.isJsonPrimitive }?.asJsonPrimitive?.takeIf { it.isString }?.asString,
                    ...
                    duration = obj.get("lengthSeconds")?.takeIf { it.isJsonPrimitive }?.asJsonPrimitive?.takeIf { it.isString }?.asString,
