...
class FollowedVideosDataDeserializer : JsonDeserializer<FollowedVideosDataResponse> {
    ...
    override fun deserialize(json: JsonElement, typeOfT: Type, context: JsonDeserializationContext): FollowedVideosDataResponse {
        ...
        val dataJson = json.asJsonObject.get("data").asJsonObject.get("currentUser").asJsonObject.get("followedVideos").asJsonObject.get("edges").asJsonArray
        ...
        val hasNextPage = json.takeIf { it.isJsonObject }?.asJsonObject?.get("data")?.takeIf { it.isJsonObject }?.asJsonObject?.get("currentUser")?.takeIf { it.isJsonObject }?.asJsonObject?.get("followedVideos")?.takeIf { it.isJsonObject }?.asJsonObject?.get("pageInfo")?.takeIf { it.isJsonObject }?.asJsonObject?.get("hasNextPage")?.takeIf { it.isJsonPrimitive }?.asJsonPrimitive?.takeIf { it.isBoolean }?.asBoolean
