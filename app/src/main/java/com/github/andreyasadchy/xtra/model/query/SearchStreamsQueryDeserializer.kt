...
class SearchStreamsQueryDeserializer : JsonDeserializer<SearchStreamsQueryResponse> {
    ...
    override fun deserialize(json: JsonElement, typeOfT: Type, context: JsonDeserializationContext): SearchStreamsQueryResponse {
        ...
        val dataJson = json.asJsonObject.get("data").asJsonObject.get("searchStreams").asJsonObject.get("edges").asJsonArray
        ...
        val hasNextPage = json.takeIf { it.isJsonObject }?.asJsonObject?.get("data")?.takeIf { it.isJsonObject }?.asJsonObject?.get("searchStreams")?.takeIf { it.isJsonObject }?.asJsonObject?.get("pageInfo")?.takeIf { it.isJsonObject }?.asJsonObject?.get("hasNextPage")?.takeIf { it.isJsonPrimitive }?.asJsonPrimitive?.takeIf { it.isBoolean }?.asBoolean
