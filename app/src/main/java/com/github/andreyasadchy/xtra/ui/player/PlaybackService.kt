...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        val player = ExoPlayer.Builder(this).apply {
            ...
            setAudioAttributes(
                ...
                prefs().getBoolean(C.PLAYER_AUDIO_FOCUS, false)
            )
                ...
                when (item) {
                    ...
                    is Video -> {
                        (item as Video).let { item ->
                            ...
                            player.prepare()
                            player.playWhenReady = true
                            player.seekTo(if (prefs().getBoolean(C.PLAYER_USE_VIDEOPOSITIONS, true)) getPosition() else 0)
                            setVideoQuality()
                        }
                    }
                    is Clip -> {
                        ...
                        return when (customCommand.customAction) {
                            START_STREAM -> {
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                                    ...
                                    (session.player as? ExoPlayer)?.setMediaSource(
                                        HlsMediaSource.Factory(
                                            DefaultDataSource.Factory(
                                                this@PlaybackService,
                                                OkHttpDataSource.Factory(okHttpClient).apply {
                            ...
                            TOGGLE_DYNAMICS_PROCESSING -> {
                                ...
                                Futures.immediateFuture(SessionResult(SessionResult.RESULT_SUCCESS, bundleOf(
                                    RESULT to dynamicsProcessing?.enabled
                                )))
                            }
                            TOGGLE_PROXY -> {
                                if (!stopProxy) {
                            ...
                            GET_QUALITY_TEXT -> {
                                Futures.immediateFuture(SessionResult(SessionResult.RESULT_SUCCESS, bundleOf(
                                    RESULT to qualities.getOrNull(qualityIndex)
                                )))
                            }
                            GET_MEDIA_PLAYLIST -> {
                                Futures.immediateFuture(SessionResult(SessionResult.RESULT_SUCCESS, bundleOf(
    ...
    private fun toggleProxy(enable: Boolean) {
        ...
        mediaSession?.player?.currentMediaItem?.let { item ->
            ...
            (mediaSession?.player as? ExoPlayer)?.setMediaSource(
                HlsMediaSource.Factory(
                    DefaultDataSource.Factory(
                        this,
